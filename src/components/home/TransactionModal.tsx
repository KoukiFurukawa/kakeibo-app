...
const TransactionModal: React.FC<ITransactionModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await Promise.race([onSubmit(transactionData), firstAttemptTimeout]);
        ...
        console.log(
          "最初の試行がタイムアウトしました。セッションを更新して再試行します。",
        );
        ...
        // セッション更新後に再試行
        await Promise.race([onSubmit(transactionData), finalTimeoutPromise]);
