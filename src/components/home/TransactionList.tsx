        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {transactions.length === 0 ? (
              <tr>
                <td
                  className="px-3 py-4 sm:px-6 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500"
                  colSpan={onEditTransaction ? 5 : 4}
                >
                  {loading
                    ? "読み込み中..."
                    : `${activeTab === "expense" ? "支出" : "収入"}データがありません`}
                </td>
              </tr>
            ) : (
            ...
            {transactions.map((transaction) => (
              ...
              >
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`font-medium ${transaction.is_income ? "text-green-600" : "text-red-600"} text-sm mr-15 mt-4`}
                  >
                    {transaction.is_income ? "+" : "-"}¥
                    {transaction.amount.toLocaleString()}
                  </span>
