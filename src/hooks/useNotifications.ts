...
export function useNotifications() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      // 現在有効な通知IDのリスト
      const activeNotificationIds = (notificationsData || []).map(
      ...
      // 既読状態をマージ
      const notificationsWithReadStatus = (notificationsData || []).map(
        (notification) => {
  ...
  const cleanupReadNotifications = (readIds: string[], activeIds: string[]) => {
    ...
    try {
      ...
      if (validReadIds.length < readIds.length) {
        console.log(
          `不要な通知ID ${readIds.length - validReadIds.length}件をクリーンアップします`,
        );
