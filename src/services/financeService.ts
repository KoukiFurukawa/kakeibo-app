...
export class FinanceService {
  ...
  static async updateUserFinance(
    userId: string,
    updates: Partial<IUserFinance>,
  ): Promise<IUserFinance | null> {
    return retryWithBackoff(async () => {
      const { data, error } = await supabase
        .from("finance")
        .update(updates)
  ...
  static async addFixedCost(
    userId: string,
    fixedCostData: Omit<IFixedCost, "id" | "created_by" | "created_at">,
  ): Promise<IFixedCost | null> {
    return retryWithBackoff(async () => {
      const { data, error } = await supabase
        .from("fixed_costs")
        .insert({
  ...
  static async fetchTransactions(
    ...
    const result = await retryWithBackoff(async () => {
      ...
      if (year && month) {
        const startDate = new Date(year, month - 1, salary_day + 1)
          .toISOString()
          .split("T")[0];
        const endDate = new Date(year, month, salary_day)
          .toISOString()
          .split("T")[0];
  ...
  static async addTransaction(
    userId: string,
    transactionData: Omit<IUserTransaction, "id" | "created_by" | "created_at">,
  ...
  static async updateTransaction(
    userId: string,
    id: string,
    transactionData: Partial<IUserTransaction>,
  ): Promise<boolean> {
    const result = await retryWithBackoff(async () => {
      const { error } = await supabase
        .from("transactions")
