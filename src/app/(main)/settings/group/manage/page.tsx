...
export default function ManageGroupPage() {
  ...
  const { user, userGroup, groupMembers, loading, refreshUserGroup } =
    useUser();
  ...
  const handleUpdateGroup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const success = await GroupService.updateUserGroup(
        user.id,
        userGroup.id,
        {
  ...
  const handleRemoveMember = async (memberId: string) => {
    ...
    try {
      ...
      const success = await GroupService.removeGroupMember(
        userGroup.author_user_id,
        userGroup.id,
        memberId,
      );
      if (success) {
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {userGroup.created_at
                    ? new Date(userGroup.created_at).toLocaleString("ja-JP")
                    : "不明"}
                </p>
              </div>
            </div>
