...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const [notificationSettings, setNotificationSettings] =
    useState<IUserNotificationSettings | null>(null);
  ...
  const validateSession = async (): Promise<boolean> => {
    try {
      ...
      const tokenExpirationTime = new Date(
        (session.access_token.split(".")[1] || "") as string,
      ).getTime();
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        ...
        const [profile, settings, finance, costs, userGroup] =
          await Promise.all([
        ...
        const userTransactions = await FinanceService.fetchTransactions(
          user.id,
          year,
          month,
          salaryDay,
